...
<table width="100%">
    ...
    WHILE (list($id_rc, $daterc, $type, $coordinates, $victoire, $metal_cdr, $cristal_cdr) = $db->sql_fetch_row($query_info)) {
        ...
        <tr>
            ...
            <th><?php $query_Def1 = $db->sql_query("SELECT player FROM " . TABLE_HOFRC_DEFENCE . " WHERE `id_rc`=" . $id_rc . " AND round = 1 GROUP BY player");
...
<table width="100%">
    ...
    WHILE (list($id_rc_pub, $daterc_pub, $type_pub, $coordinates_pub, $victoire_pub, $metal_cdr_pub, $cristal_cdr_pub) = $db->sql_fetch_row($query_info_pub)) {
        ...
        <tr>
            ...
            <th><?php $query_Def_pub = $db->sql_query("SELECT `player` FROM " . TABLE_HOFRC_DEFENCE . " WHERE `id_rc`=" . $id_rc_pub . " AND round = 1 GROUP BY player");
